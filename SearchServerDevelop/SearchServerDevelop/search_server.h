...
std::tuple<std::vector<std::string_view>, DocumentStatus> SearchServer::MatchDocument( ExecutionPolicy&& policy, const std::string_view& raw_query, int document_id ) const
{
    ...
    auto& pl_words = query.plus_words;

    std::vector<std::string_view> matched_words;
    matched_words.reserve( pl_words.size() );

    auto erase_iter = std::copy_if( policy,
                                    pl_words.begin(),
                                    pl_words.end(),
                                    matched_words.begin(),
                                    [this, document_id] ( const std::string_view& word )
                                    {
                                        return word_to_document_freqs_.count( static_cast< std::string >( word ) ) > 0 &&
                                            word_to_document_freqs_.at( static_cast< std::string >( word ) ).count( document_id ) > 0;
                                    } );
    if (matched_words.size() > 0 )
        matched_words.erase( erase_iter, matched_words.end() );
...
SearchServer::Query SearchServer::ParseQuery( ExecutionPolicy&& policy, const std::string_view& text ) const
{
    ...
    if constexpr ( !std::is_same_v<std::decay_t<ExecutionPolicy>, std::execution::parallel_policy> )
    {
        SortAndUnique( min_words );
        SortAndUnique( pls_words );
    }
